...
public class ABObjectPool<T> where T : MonoBehaviour {
	...
	public ABObjectPool (int polledAmount = 10, GameObject[] reference = null, InitObject initMethod = null) {
		//System.Console.WriteLine("=== entering ABObjectPool.ABObjectPool() ===");
		//System.Console.WriteLine("ABObjectPool.ABObjectPool(): polledAmount=" + polledAmount 
								 //+ ", reference=" + reference + ", initMethod=" + initMethod);
		...
		for(int i = 0; i < PolledAmount; i++) {
			...
			if (reference == null) {
				...
				//System.Console.WriteLine("ABObjectPool.ABObjectPool(): reference.Length=" + reference.Length);
